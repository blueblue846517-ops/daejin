...
$idxTruncated = $idxLines[0..729]

$portLines = Get-Content -Path ".\portfolio.html" -Encoding UTF8
$portStartIdx = -1
$portEndIdx = -1

for ($i = 0; $i -lt $portLines.Count; $i++) {
    if ($portLines[$i] -match "MODAL 4: 개인정보처리방침") {
        $portStartIdx = $i - 1
    }
    if ($portLines[$i] -match "<!-- Supabase Integration for Portfolio Page -->") {
        $portEndIdx = $i - 1
        break
    }
}

$portCommon = $portLines[$portStartIdx..$portEndIdx]
...
$finalContent = $idxTruncated + $portCommon + $indexScript.Split("`n").Replace("`r", "")
Set-Content -Path ".\index.html" -Value $finalContent -Encoding UTF8
Write-Host "Index.html restored completely!"
